...
为“凯瑞甘生存”网站提供游戏排行榜能力：网站通过 Worker API 获取由 KV 托管的排行榜数据，并在前端以中文界面展示“幸存者榜 / 凯瑞甘榜”的 Top 50 排名与更新时间。

## Requirements

### Requirement: 提供排行榜数据接口

系统 MUST 提供 `GET /api/leaderboard` 接口，用于返回“精简后的排行榜数据”。该接口 MUST 从 Cloudflare KV 读取固定 key：`bridge_cn.json`，并从中提取 `generated_at` 与 `leaderboard` 数据结构进行裁剪/重组后返回；接口 MUST NOT 直接透传原始 `bridge_cn.json` 内容。
...
- `rank`: number（从 1 开始）
- `display_name`: string
- `identity`: string（玩家战网标识，用于详情弹窗展示）
...
- 两个榜单区域（或两个 Tab）：
  - `幸存者` 榜
  - `凯瑞甘` 榜
- 每个榜单以 MMR 从高到低展示 Top 50 条目。
- 榜单列表中每条至少展示：名次（`rank`）、玩家名（`display_name`）、MMR（`mmr`）。
- 点击榜单条目后，页面 MUST 通过弹窗展示该玩家的“句柄（`handles`）/ 玩家战网标识（`identity`）”等信息。

页面 MUST NOT 在榜单列表中展示 `handles`。
页面 MUST NOT 展示 `games_played`。

#### Scenario: 用户打开排行榜页面
- **WHEN** 用户访问 `/leaderboard`
- **THEN** 页面请求 `GET /api/leaderboard` 并在成功后渲染“更新时间”与两个中文榜单（幸存者/凯瑞甘），且不展示 `games_played`
