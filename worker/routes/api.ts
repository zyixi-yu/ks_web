...
api.get("/role-map", async (c) => {
  ...
  const parsed = await kvGetJson(c.env.KS_KV, key);
  if (!parsed) return jsonError(c, 404, `KV key not found: ${key}`);
  if (!parsed || typeof parsed !== "object") return jsonError(c, 500, "bridge_cn.json format error");

  const obj = parsed as Record<string, unknown>;
  const generatedAt = obj["generated_at"];
  const roleMap = obj["role_id_to_name"];
  if (typeof generatedAt !== "string" || !generatedAt.trim()) return jsonError(c, 500, "bridge_cn.json missing generated_at");
  if (!roleMap || typeof roleMap !== "object") return jsonError(c, 500, "bridge_cn.json missing role_id_to_name");

  const payload = { generated_at: generatedAt, role_id_to_name: roleMap };
  const resp = jsonOk(c, payload, { cacheControl: apiCacheControl() });
  c.executionCtx.waitUntil(defaultCache().put(cacheKey, resp.clone()));
  return resp;
});
